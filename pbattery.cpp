...
pBattery::pBattery(QWidget *parent) :QWidget(parent),ui(new Ui::pBattery)
{
    ui->setupUi(this);
    setupBatteryPage();
    CPrime::ThemeFunc::addDropShadow(ui->overviewF,40);
    CPrime::ThemeFunc::addDropShadow(ui->morinfoF,40);
}
...
void pBattery::on_batteriesList_currentIndexChanged(int index)
{
    ...
    QStringListModel *infoModel = new QStringListModel(CPrime::StringFunc::fStringList(left, right, ui->batProperties->font()));

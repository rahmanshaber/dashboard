...
presources::presources(QWidget *parent)
    : QWidget(parent), ui(new Ui::presources), im(InfoManager::ins()),
      mTimer(new QTimer(this)),
      mChartCpu(new HistoryChart(tr("History of CPU"), im->getCpuCoreCount(),
                                 nullptr, this)),
      mChartCpuLoadAvg(new HistoryChart(tr("History of CPU Load Averages"), 3,
                                        nullptr, this)),
      mChartDiskReadWrite(new HistoryChart(tr("History of Disk Read Write"), 2,
                                           new QCategoryAxis, this)),
      mChartMemory(new HistoryChart(tr("History of Memory"), 2, nullptr, this)),
      mChartNetwork(new HistoryChart(tr("History of Network"), 2,
                                     new QCategoryAxis, this)) {
  ui->setupUi(this);
...
presources::~presources() { delete ui; }

void presources::init() {
  mChartCpu->setYMax(100);
  mChartMemory->setYMax(100);

  QList<QWidget *> widgets = {mChartCpu, mChartCpuLoadAvg, mChartDiskReadWrite,
                              mChartMemory, mChartNetwork};
...
void presources::updateCpuLoadAvg() {
  ...
  for (int j = 0; j < seriesList.count(); ++j) {
    ...
    seriesList.at(j)->setName(
        tr("%1 Minute Avarage: %2").arg(minutes[j]).arg(avg));
...
void presources::updateNetworkChart() {
  ...
  int max = static_cast<int>(qMax(max_RXbytes, max_TXbytes)); // cast
...
void presources::updateMemoryChart() {
  ...
  if (im->getSwapTotal()) // aritmetic exception control
    percent = (static_cast<double>(im->getSwapUsed()) /
               static_cast<double>(im->getSwapTotal())) *
              100.0;

  seriesList.at(0)->insert(0, QPointF(0, percent));
  seriesList.at(0)->setName(
      tr("Swap: %1 (%2%) %3")
  ...
  // Memory
  double percent2 = (static_cast<double>(im->getMemUsed()) /
                     static_cast<double>(im->getMemTotal())) *
                    100.0;

  seriesList.at(1)->insert(0, QPointF(0, percent2));
  seriesList.at(1)->setName(
      tr("Memory: %1 (%2%) %3")
...
void presources::updateCpuChart() {
  ...
  for (int j = 0; j < seriesList.count(); j++) {
    ...
    for (int i = 0; i < (second < 61 ? second : 61); i++) {
      ...
      seriesList.at(j)->replace(
          i, static_cast<double>(i + 1),
          static_cast<double>(seriesList.at(j)->at(i).y()));
    }
